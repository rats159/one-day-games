...
clay_raylib_render :: proc(
	...
	for i in 0 ..< render_commands.length {
		...
		if render_command.userData != nil {
			transform := (^Transform)(render_command.userData)^
			rlgl.PushMatrix()
			rlgl.Translatef(bounds.x + bounds.width / 2, bounds.y + bounds.height / 2, 0)
			rlgl.Scalef(transform.scale.x, transform.scale.y, 1)
			rlgl.Rotatef(transform.rotation, 0, 0, 1)
			rlgl.Translatef(transform.translate.x, transform.translate.y, 0)
			rlgl.Translatef(-(bounds.x + bounds.width / 2), -(bounds.y + bounds.height / 2), 0)
		}
		defer if render_command.userData != nil {
			rlgl.PopMatrix()
		}
		...
		case .Rectangle:
			...
			if config.cornerRadius.topLeft > 0 {
				...
				fmt.println(radius, config.cornerRadius.topLeft)
				draw_rect_rounded(
					bounds.x,
					bounds.y,
					bounds.width,
...
draw_rect_rounded :: proc(x, y, w, h: f32, radius: f32, color: clay.Color) {
	if radius >= 0.999 && abs(w - h) < 0.01 {
		rl.DrawCircleV({x + w / 2, y + h / 2}, w/2, clay_color_to_rl_color(color))
	} else {
